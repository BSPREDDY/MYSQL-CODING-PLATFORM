...
// 		// Run each test case without using try/catch for control flow
// 		for (const testCase of testCases) {
// 			await clearAllTableData(); // Clear all table data before running the test case
...
// 					testCaseId: testCase.id,
// 					passed: false,
// 					actualOutput: { error: result.error || "Query execution failed" },
// 				});
// 				continue;
// 			}

// 			// We have a successful query execution, now check output matching
// 			let passed = true;
// 			let outputMismatchError = null;

// 			// Compare output with expected output if available
// 			if (testCase.expectedOutput && result.data) {
// 				const expectedOutput = JSON.parse(testCase.expectedOutput);
...
// 				// If not passed, create an output mismatch error message
// 				if (!passed) {
// 					outputMismatchError = {
...
// 			// Add result to the test results array
// 			testResults.push({
// 				testCaseId: testCase.id,
// 				passed,
// 				actualOutput: outputMismatchError || result.data,
// 			});
...
export async function runCode({
	...
	try {
		...
		for (const testCase of testCases) {
			try {
				// Clean up from previous test - delete all tables before creating new ones
				await deleteAllTables();

				// Execute the boilerplate setup for each test case
				const boilerplateResult = await executeUserQuery(boilerplate);
				...
				if (!boilerplateResult.success) {
					testResults.push({
						...
						passed: false,
						actualOutput: {
							error: boilerplateResult.error || "Boilerplate setup failed",
						},
					});
					continue;
				}
