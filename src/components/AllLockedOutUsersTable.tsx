...
// 		setProcessingUsers((prev) => new Set(prev).add(key));

// 		try {
// 			const response = await fetch(`/api/contests/${contestId}/grant-reentry`, {
// 				method: "POST",
// 				headers: {
...
// 				// Remove the user from the state (optimistic UI update)
...
// 				toast({
// 					title: "Error",
// 					description: errorData.error || "Failed to grant re-entry",
// 					variant: "destructive",
// 				});
// 			}
// 		} catch (error) {
...
// 					{users.length > 0 ? (
// 						users.map((user) => {
// 							const key = `${user.userId}-${user.contestId}`;
// 							return (
// 								<TableRow key={key} id={`user-row-${key}`}>
// 									<TableCell>
// 										<div>
...
// 									<TableCell>{user.contestTitle}</TableCell>
// 									<TableCell>
...
// 											}
// 											disabled={processingUsers.has(key)}
// 										>
// 											{processingUsers.has(key)
// 												? "Processing..."
// 												: "Grant Re-entry"}
// 										</Button>
// 									</TableCell>
